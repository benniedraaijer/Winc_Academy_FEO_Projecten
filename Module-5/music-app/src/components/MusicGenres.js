...
export default function MusicGenres() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      const rock = await database.collection('fav-songs').where('genre', '==', 'Rock').get();
      const country = await database.collection('fav-songs').where('genre', '==', 'Country').get();
      const pop = await database.collection('fav-songs').where('genre', '==', 'Pop').get();
      const nl = await database.collection('fav-songs').where('genre', '==', 'Nl').get();
      const hiphop = await database.collection('fav-songs').where('genre', '==', 'Hip Hop').get();
      const soul = await database.collection('fav-songs').where('genre', '==', 'Soul').get();
      const jazz = await database.collection('fav-songs').where('genre', '==', 'Jazz').get();
      const blues = await database.collection('fav-songs').where('genre', '==', 'Blues').get();
      const metal = await database.collection('fav-songs').where('genre', '==', 'Metal').get();
      const punk = await database.collection('fav-songs').where('genre', '==', 'Punk').get();
    ...
    <>
      ...
      <div id='genre-container'>
        <div className='genre-box' id='Rock'>
          Rock
          {rock.map((song) => (
            <div className='genre-row' id={song.id} key={song.song + new Date()}>
